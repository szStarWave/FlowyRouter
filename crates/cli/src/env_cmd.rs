...
pub struct EnvConfig {
    ...
    pub cli_gateway_bin: Option<String>,
}
...
fn build_report(config_override: &Option<PathBuf>) -> Result<EnvReport> {
    ...
    let gateway_bin = daemon_ctl::resolve_gateway_bin(&settings)
    ...
    Ok(EnvReport {
        paths: EnvPaths {
            ...
            app_dir: flowy_config::app_dir()?.display().to_string(),
        ...
        config: EnvConfig {
            ...
            edge_upstream,
            cloud_upstream,
            cli_gateway_bin: file.cli.gateway_bin.clone(),
        },
        runtime: EnvRuntime {
